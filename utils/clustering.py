...
def _fit_3d_projection(X: np.ndarray, prefix: str) -> tuple[pd.DataFrame, PCA | None]:
    """
    Fit a 3-axis PCA projection when the matrix supports it, otherwise pad the
    missing axes with zeros so downstream visualizations do not crash on tiny inputs.
    """
    ...
    for axis_idx, axis_name in enumerate(["x", "y", "z"][:max_components]):
        projection[f"{prefix}_{axis_name}"] = transformed[:, axis_idx]
