...
def _fit_3d_projection(X: np.ndarray, prefix: str) -> tuple[pd.DataFrame, PCA | None]:
    """
    Fit a 3-axis PCA projection when the matrix supports it, otherwise pad
    the missing axes with zeros so downstream visualization code stays stable.
    """
    projection = pd.DataFrame(index=np.arange(len(X)), columns=[f"{prefix}_x", f"{prefix}_y", f"{prefix}_z"], dtype=float)
    projection.loc[:, :] = 0.0

    max_components = min(3, X.shape[0], X.shape[1])
    if max_components <= 0:
        return projection, None

    reducer = PCA(n_components=max_components, random_state=42)
    transformed = reducer.fit_transform(X)
    axis_names = [f"{prefix}_x", f"{prefix}_y", f"{prefix}_z"]
    for axis_idx, axis_name in enumerate(axis_names[:max_components]):
        projection[axis_name] = transformed[:, axis_idx]
...
def run_kmeans(df: pd.DataFrame, user_history: dict, k: int = 8):
    ...
    # PCA display coordinates with graceful fallback for tiny datasets.
    pca_projection, pca = _fit_3d_projection(X_scaled, "pca")
    df[["pca_x", "pca_y", "pca_z"]] = pca_projection[["pca_x", "pca_y", "pca_z"]].to_numpy()
    components = pca.components_ if pca is not None else np.zeros((0, X_scaled.shape[1]))
    axis_labels = [
    ...
    pca_axis_fallbacks = ["Restaurant Style", "Quality & Popularity", "Mixed Factors"]
    pca_summary_fallbacks = [
        "Primary clustering directions",
        "Secondary clustering directions",
        "Tertiary clustering directions",
    ]
    pca.axis_labels_ = axis_labels + pca_axis_fallbacks[len(axis_labels):]
    pca.component_summaries_ = component_summaries + pca_summary_fallbacks[len(component_summaries):]

    # Cluster-oriented display coordinates based on centroid distances.
    centroid_distances = kmeans.transform(X_cluster)
    scaled_centroid_distances = StandardScaler().fit_transform(centroid_distances)
    cluster_view_projection, _ = _fit_3d_projection(scaled_centroid_distances, "cluster_view")
    df[["cluster_view_x", "cluster_view_y", "cluster_view_z"]] = cluster_view_projection[
        ["cluster_view_x", "cluster_view_y", "cluster_view_z"]
    ].to_numpy()

    # t-SNE 3D coordinates for visualization only.
    if 5 < len(df) <= TSNE_MAX_ROWS:
