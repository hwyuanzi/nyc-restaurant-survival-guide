"""
utils/user_profile.py — User profile management and preference scoring.

Supports creating, saving, and loading user profiles with cuisine, borough,
budget, vibe, and spice preferences. Scores restaurants against user
...
def render_profile_sidebar():
    ...
    profile_labels = {pid: profiles[pid]["name"] for pid in profile_ids}
    ...
    favorite_cuisines = st.multiselect("Favorite cuisines", options=CUISINE_OPTIONS, default=profile.get("favorite_cuisines", []))
    preferred_boroughs = st.multiselect("Preferred boroughs", options=BOROUGH_OPTIONS, default=profile.get("preferred_boroughs", []))
    ...
    favorite_vibes = st.multiselect("Favorite vibes", options=VIBE_OPTIONS, default=profile.get("favorite_vibes", []))
