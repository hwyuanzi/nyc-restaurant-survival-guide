...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### Clustering Reference")
    if "pca_show_k_explainer" not in st.session_state:
        st.session_state["pca_show_k_explainer"] = False
    if st.button("📘 Explain Optimal K (Deep Dive)"):
        st.session_state["pca_show_k_explainer"] = not st.session_state["pca_show_k_explainer"]
    if st.session_state["pca_show_k_explainer"]:
        st.markdown(
            """
